...
3. **Manifest Generation:** Create a complete `manifest.json` that follows the "Intelligence + Stubs" architecture (ensuring file paths do NOT include the `stubs/` prefix).
4. **Registry Entry:** Provide the JSON snippet for the root `registry.json`.
